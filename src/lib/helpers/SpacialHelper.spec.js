...
import Registry from 'store/registry/Registry';
...
describe("getCombineMatrix",()=>{
    ...
    test('In X',()=>{
        let obj1={id:'1',transform:'1,2,0,1,20,20',matrix: new Matrix('1,2,0,1,20,20') }
        let obj2={id:'2',parent:'1',transform:'1,2,0,1,30,40',matrix: new Matrix('1,2,0,1,30,40')}
...
describe("Testing scale to center ",()=>{
    ...
    test('Test scale 2',()=>{
        ...
        let testMatrix=new Matrix('1,0,0,1,20,20')
        testMatrix=testMatrix.translate(cx,cy)
        testMatrix=testMatrix.scale(scale)
        testMatrix=testMatrix.translate(-cx,-cy)
...
describe("Test Resize Obj",()=>{
    ...
    test('TOP LEFT',()=>{
        ...
        state.box={w:20,h:20}
        let result=SpacialHelper.resizeObject(10,10,Consts.MODE_RUBER_BAND_RESIZE_UL,state)
    ...
    test('TOP RIGHT',()=>{
        ...
        state.selectedMtx=new Matrix();
        state.box={w:20,h:20}
        let result=SpacialHelper.resizeObject(10,10,Consts.MODE_RUBER_BAND_RESIZE_UR,state)
    ...
    test('BOTTOM LEFT',()=>{
        ...
        state.selectedMtx=new Matrix();
        state.box={w:20,h:20}
        let result=SpacialHelper.resizeObject(10,10,Consts.MODE_RUBER_BAND_RESIZE_DL,state)
        expect(result.matrix.e).toBe(-10)
    ...
    test('BOTTOM RIGHT',()=>{
        ...
        state.selectedMtx=new Matrix();
        state.box={w:20,h:20}
        let result=SpacialHelper.resizeObject(10,10,Consts.MODE_RUBER_BAND_RESIZE_DR,state)
        expect(result.matrix.e).toBe(0)
        expect(result.matrix.f).toBe(0)
        expect(result.box.w).toBe(10)
        expect(result.box.h).toBe(10)
